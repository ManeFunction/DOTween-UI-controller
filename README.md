...
1. Seven pre-configured tweens for all essential UI animation operations: **Move** (X/Y), **Scale** (X/Y), **Rotate** (Z only), **Color** (for Images and other MaskableGraphic components), and **Fade** (for the alpha property of CanvasGroup).
...
1. Flexible Unity lifecycle integration, allowing tweens to pause on `OnDisable()`, for example, or to be reused and replayed on `OnEnable()`.
...
1. Easing options with support for both custom curves and built-in mathematical easing functions.
...
1. The ability to replay animations directly from the editor for testing and fine-tuning without relaunching your game (available only in `Play mode` due to `DOTween` limitations).
...
`OpenUPM` is the best place to find and manage open-source Unity packages, providing a seamless package-management
experience - including dependency resolution and updates - right from Unity’s `Package Manager`.

For CLI operations on Windows, I recommend using `Git Bash` (`MINGW`) because it provides a Unix-like environment
...
1. Install the package via `OpenUPM`:
   - Open a CLI and navigate to your Unity project folder: `cd /path/to/your/project`.
